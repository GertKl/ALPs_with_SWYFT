...
if __name__ == "__main__":
    ...
    for item in config_list:
        ...
        if len(config_argument_name_and_value)==2:
            ...
            if len(config_dict_item)>1: is_1d=False
            
            for row, item_row in enumerate(config_dict_item):
                
                config_dict_item[row] = item_row.split("::")
                if len(config_dict_item[row])>1: is_1d=False
                
            config_dict[config_argument_name_and_value[0]] = config_dict_item[0][0] if is_1d else config_dict_item
