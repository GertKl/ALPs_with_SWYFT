...
if __name__ == "__main__":
    ...
    A.configure_model(
        ...
        noise="poisson",
        log_params=is_log,
        null_params = config_dict['null_params'],
        param_names = config_dict['param_names'],
        param_units = config_dict['param_units'],
        ALP_seed=eval(config_dict['ALP_seed']),
        floor=eval(config_dict['floor']),
        floor_obs=eval(config_dict['floor_obs']), # not reflected in training set of all_larger_bounds
        logcounts=True,
        residuals=True
    )
