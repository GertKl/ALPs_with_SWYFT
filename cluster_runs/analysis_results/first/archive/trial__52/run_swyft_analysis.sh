...
if [ $update_params == 1 ] || [ $update_model == 1 ] ; then
	
	echo
	echo "Converting new analysis variables"
	echo "------------------------------------------------------"
	echo
	...
	for file_name in $(find ${results_dir}/archive/trial__$(($i - 1))/${item} -maxdepth 1 -type f )
	do
		found=0
		for item in "${configuration_scripts[@]}"
		do
			if [ "${results_dir}/archive/trial__$(($i - 1))/${item}" == "$file_name" ]; then
				found=1
				break
			fi
	    	done
	...
	echo
	echo "Imported variables from run number $(($i - 1))"
	echo
	# copy configuration files back from last trial in archive 
